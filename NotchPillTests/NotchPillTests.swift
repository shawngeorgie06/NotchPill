...
struct TranscriptTurnTests {
    ...
    @Test("Claude response usage remains a response measurement, not a quota")
    func claudeCodeUsage() {
        let transcript = #"{"timestamp":"2026-07-31T17:00:00Z","type":"assistant","message":{"usage":{"input_tokens":12,"output_tokens":3456,"cache_read_input_tokens":7890,"cache_creation_input_tokens":100}}}"#
        let usage = AgentSessionScanner.claudeCodeUsage(in: transcript)
        #expect(usage?.responseLabel == "3.5K out · 12 in")
        #expect(usage?.cacheLabel == "8.0K cache")
